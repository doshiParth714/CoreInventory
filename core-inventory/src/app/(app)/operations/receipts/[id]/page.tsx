...
import { Printer, X, CheckCircle, ChevronLeft, AlertTriangle, Plus } from "lucide-react";
...
export default function ReceiptDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!data) return <div className="p-6 text-slate-400">Receipt not found.</div>;
  ...
  function handleAction() {
    ...
    else if (data.status === "READY" && user) actions.applyReceipt(id, user.loginId);
  }
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-400 mt-0.5">Receipt · Incoming Goods</p>
        </div>
            ...
            <button
              onClick={() => {
                if (newProductId && newQty) {
                  actions.addMoveLine(id, newProductId, Number(newQty), data.toLocationId ?? undefined);
                  setNewProductId("");
                  setNewQty("");
                }
              }}
