...
export default function ReceiptsListPage() {
  ...
  const items = state.pickings
    .filter((p) => p.operationType === "RECEIPT")
  ...
  function handleCreate() {
    const id = actions.createPicking("RECEIPT");
        ...
        <div className="rounded-xl border border-white/10 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="border-b border-white/10 bg-white/3">
              <tr>
                {["Reference", "Contact / From", "To Location", "Scheduled Date", "Status"].map((h) => (
            ...
            <tbody className="divide-y divide-white/5">
              {items.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500">No receipts found.</td></tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id} onClick={() => router.push(`/operations/receipts/${item.id}`)} className="cursor-pointer hover:bg-white/5 transition">
                    ...
                    <td className="px-4 py-3 text-slate-400">{item.toLocation?.name ?? "—"}</td>
              ...
              <div className="flex-1 flex flex-col gap-3">
                {items.filter((i) => i.status === colStatus).map((item) => (
                  <div
                    ...
                    className="rounded-lg border border-white/10 bg-white/3 p-4 cursor-pointer hover:bg-white/5 transition space-y-3"
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{item.toLocation?.name ?? "Unknown Location"}</p>
                    </div>
